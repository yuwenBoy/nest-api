...
export class RoleController {
  ...
  @Post('/setRoles')
  setRoles(@Body() request: UserRoleDto,@CurrentUser() userInfo:UserInfoDto): Promise<any> {
    Logger.log('设置角色接受参数：' + request);
    try {
      if (!request.userId) {
        return;
      } else {
        return this.roleService.saveUserRole(request,userInfo);
      }
    } catch (error) {
