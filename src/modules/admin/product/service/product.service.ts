...
export class ProductService {
  ...
  async getPropertyInfo(productId: number): Promise<any> {
    const productProperties = await this.productSpecAttrRelationRepository.find(
      { where: { productId } },
    );
    // 提取所有唯一的属性值 ID
    ...
    productProperties.forEach((spec) => {
      try {
        ...
        try {
          if (
            spec.attributeOptionJson &&
            typeof spec.attributeOptionJson === 'string'
          ) {
  ...
  async detail(productId: number): Promise<any> {
    try {
      ...
      const product_spea = await this.productSpecRepository.find({
        where: { productId },
      });

      const categories = (
        await this.productCategoryRepository.find({ where: { productId } })
      ).map((y) => y.productCategoryId);
      const productDynamicEntity =
        await this.productDynamicAttributeRepository.find({
          where: { productId },
        });
        ...
        productDynamicEntity.map(async (item) => {
          if (Number(item.attributeValueId)) {
            const attributeValue =
              await this.productDynamicAttributeValueRepository.findOne({
                where: { id: Number(item.attributeValueId) },
              });
            return {
              ...item, // 保留原始对象的其他属性
      ...
      const groupedByAttributeId = productDynamicWithParentIds.reduce(
        (acc, item) => {
      ...
      const sortedGroups = Object.values(groupedByAttributeId).map(
        (
          group: {
            parentId: number;
   ...
   */
  async fetchProperties(storeId): Promise<any> {
    try {
      if (storeId>0) {
        const properties = await this.productSpecAttrRepository.find({
          where: storeId>0?{ businessId :storeId}:{},
        });
        interface ProductSpecAttrEntityWithDetails
          extends ProductSpecAttrEntity {
      ...
      return {
        success: false,
        message: '根据商家ID查询属性信息失败，原因：' + error,
      };
    }
  }
        ...
        async (transactionalEntityManager) => {
          ...
          if (params.type == 1) {
            ...
            for (const productId of productIds) {
              const productGroupRelation = transactionalEntityManager.create(
                ProductGroupRelationEntity,
                {
                  productId,
          ...
          // 批量改打包费
            ...
            const productSpec = await this.productSpecRepository.find({
              where: {
                productId: In(productIds),
              },
            });
            for (const spec of productSpec) {
              const specEntity = transactionalEntityManager.create(
                ProductSpecEntity,
                {
          ...
          // 批量改份量规格
            ...
            await transactionalEntityManager.delete(ProductSpecEntity, {
              productId: In(productIds),
            });
          ...
          // 批量改属性规格
            ...
            const products = await transactionalEntityManager.find(
              ProductEntity,
              {
            ...
            const savedProductSpecs = await transactionalEntityManager.find(
              ProductSpecEntity,
              {
            ...
            if (params.properties.length > 0) {
              const { savedProperties, savedPropertyValues } =
                await this.savePropertyAndPropertieOption(
              ...
              for (const product of products) {
                const productSpecForProduct = savedProductSpecs.filter(
                  (t) => t.productId == product.id,
                );

                // 将属性和属性选项转换为笛卡尔积需要的格式
                const propertyOptions = savedProperties.map((item) => {
                  const propertyValues = savedPropertyValues.filter(
                    (pv) => pv.productSpecAttrId === item.id,
                  );
                ...
                propertyCombinations.forEach((propertyCombination) => {
                  ...
                  productSpecForProduct.forEach((item) => {
                    // 创建新的关联记录
                    const newRelation = transactionalEntityManager.create(
                      ProductSpecAttrRelationEntity,
                      {
                        productId: product.id,
                        productSpecId: item.id,
                        attributeOptionJson:
                          JSON.stringify(propertyCombination),
                      },
                    );
  ...
  async getStatistics(@Request() req) {
    const products = await this.productRepository.find({
      where: { storeId: req.queryParams.store_id },
    });
    const productCount = products.length; //商品总数
    const downActiveCount = products.filter((t) => t.isActive == 2)?.length; // 已下架
    const spec = await this.productSpecRepository.find({
      where: { productId: In(products.map((t) => t.id)), stock: 0 },
    });
        ...
        .andWhere(
          new Brackets((qb) => {
            if (parameter.storeId) {
              qb.andWhere('product.store_id  = :storeId', {
        ...
        .andWhere(
          new Brackets((qb) => {
            if (parameter.groupId) {
              qb.andWhere('pgr.group_id  = :groupId', {
   ...
   * 保存产品规格属性关联关系
   */
  async saveProductSpecPropertyRelation(
    transactionalEntityManager,
    savedProductSpecs,
    savedProperties,
    savedPropertyValues,
    ...
    // 删除旧的关联记录
    await transactionalEntityManager.delete(ProductSpecAttrRelationEntity, {
      productId,
    });
   ...
   * 修复版：正确处理新增/编辑属性的选项保存
   */
  async savePropertyAndPropertieOption(
    transactionalEntityManager: EntityManager,
    properties: any[],
    businessId: number,
  ): Promise<{ savedProperties: any[]; savedPropertyValues: any[] }> {
    const savedProperties: any[] = [];
    const savedPropertyValues: any[] = [];
    ...
    for (let i = 0; i < properties.length; i++) {
      ...
      if (attr.id) {
        ...
        if (existingProperty) {
          if (existingProperty.name !== attr.name) {
            // 名称变化：创建新属性
            const newProperty = transactionalEntityManager.create(
              ProductSpecAttrEntity,
              {
                name: attr.name,
                sort: i + 1,
            ...
            savedProperty = await transactionalEntityManager.save(newProperty);
            Logger.log('✅ 创建新属性（名称变化）:', savedProperty.name);
          } else {
            // 名称未变：更新现有属性
            existingProperty.sort = i + 1;
            existingProperty.businessId = businessId;
            savedProperty = await transactionalEntityManager.save(
              existingProperty,
            );
            Logger.log('✅ 更新属性:', savedProperty.name);
          }
        } else {
          throw new Error(`属性 id ${attr.id} 不存在`);
        }
      } else {
        // 新增属性
        const newProperty = transactionalEntityManager.create(
        ...
        savedProperty = await transactionalEntityManager.save(newProperty);
        Logger.log('✅ 新增属性:', savedProperty.name);
      }

      savedProperties.push({
        ...savedProperty,
        originalIndex: i, // 保留原始索引用于后续匹配
      });
    }

    // 第二步：处理属性选项
    // 查询所有相关选项（包括旧属性和新属性的）
    const allPropertyIds = savedProperties.map((p) => p.id);
    const existingPropertyValues = await transactionalEntityManager.find(
      ProductSpecAttrOptionEntity,
      {
        where: { productSpecAttrId: In(allPropertyIds) },
      },
    );
    ...
    existingPropertyValues.forEach((pv) => {
      existingValueMap.set(pv.id, pv);
    });

    // 处理每个属性的选项
    for (const savedProp of savedProperties) {
      const originalAttr = properties[savedProp.originalIndex];

      if (!originalAttr.details || originalAttr.details.length === 0) {
        continue;
      }

      for (const detail of originalAttr.details) {
        let savedValue;

        if (detail.id) {
          // 编辑现有选项
          const existingValue = existingValueMap.get(detail.id);

          if (existingValue) {
            // 检查是否属于当前属性
            if (existingValue.productSpecAttrId !== savedProp.id) {
              // 属性变化了，创建新选项
              const newValue = transactionalEntityManager.create(
                ProductSpecAttrOptionEntity,
                {
                  name: detail.name,
                  productSpecAttrId: savedProp.id,
                  saleStatus: detail.saleStatus,
                },
              );
              savedValue = await transactionalEntityManager.save(newValue);
              Logger.log('✅ 创建新选项（属性变化）:', savedValue.name);
            } else {
              // 更新现有选项
              existingValue.name = detail.name;
              existingValue.saleStatus = detail.saleStatus;
              savedValue = await transactionalEntityManager.save(existingValue);
              Logger.log('✅ 更新选项:', savedValue.name);
            }
          } else {
            throw new Error(`选项 id ${detail.id} 不存在`);
          }
        } else {
          // 新增选项
          const newValue = transactionalEntityManager.create(
          ...
          savedValue = await transactionalEntityManager.save(newValue);
          Logger.log('✅ 新增选项:', savedValue.name);
        }

        savedPropertyValues.push(savedValue);
      }
    }

    // 清理：删除不再需要的旧选项（可选）
    await this.cleanupOrphanedValues(
      transactionalEntityManager,
      allPropertyIds,
      savedPropertyValues.map((v) => v.id),
    );

    return {
      savedProperties: savedProperties.map((p) => {
        const { originalIndex, ...rest } = p;
        return rest;
      }),
      savedPropertyValues,
    };
   ...
   * 清理孤立的属性选项
   */
  private async cleanupOrphanedValues(
    transactionalEntityManager: EntityManager,
    propertyIds: number[],
    keptValueIds: number[],
  ): Promise<void> {
    const orphanedValues = await transactionalEntityManager.find(
      ProductSpecAttrOptionEntity,
      {
        where: {
          productSpecAttrId: In(propertyIds),
          id: Not(In(keptValueIds.length > 0 ? keptValueIds : [0])),
        },
      },
    );

    if (orphanedValues.length > 0) {
      Logger.log(`🗑️ 清理 ${orphanedValues.length} 个孤立选项`);
      await transactionalEntityManager.remove(orphanedValues);
    }
  }
  /**
   * 编辑产品
    ...
    return this.productRepository.manager.transaction(
      async (transactionalEntityManager) => {
        try {
          ...
          // 更新产品分组关联表
          await transactionalEntityManager.delete(ProductGroupRelationEntity, {
            ...
            if (Array.isArray(attr.attributeValue)) {
              for (const value of attr.attributeValue) {
                ...
                await transactionalEntityManager.save(
                  productDynamicAttributeRelation,
                );
              }
            } else {
              const productDynamicAttributeRelation =
          ...
          if (dto.product_spea.length > 0) {
            ...
            // 查询数据库 中已存在的产品规格记录
            const existingProductSpecs = await transactionalEntityManager.find(
              ProductSpecEntity,
              {
            ...
            dto.product_spea.forEach((spea) => {
              const existingSpec = Array.from(existingSpecMap.values()).find(
                (spec) => spec.id === spea.id,
              );
              if (existingSpec) {
                // 如果存在，更新该记录
                ...
                // 如果不存在，创建新的记录
                const newSpec = transactionalEntityManager.create(
                  ProductSpecEntity,
                  {
                    productId: savedProduct.id,
            ...
            if (updatedProductSpecs.length > 0) {
              savedProductSpecs = await transactionalEntityManager.save(
                updatedProductSpecs,
              );
            }
            ...
            savedProductSpecs = Array.from(
              new Set(savedProductSpecs.map((spec) => spec.id)),
            ).map((id) => savedProductSpecs.find((spec) => spec.id === id));
            // 删除数据库中不再需要的产品规格记录
            const specIdsToKeep = [
              ...updatedProductSpecs,
              ...newProductSpecs,
            ].map((spec) => spec.id);
            const specsToDelete = Array.from(existingSpecMap.values()).filter(
              (spec) => !specIdsToKeep.includes(spec.id),
            );
            ...
            if (dto.properties.length > 0) {
              ...
              // 保存产品规格属性关联关联关系
              await this.saveProductSpecPropertyRelation(
                transactionalEntityManager,
                savedProductSpecs,
                savedProperties,
                savedPropertyValues,
        ...
        try {
          ...
          if (dto.categories.length > 0) {
            ...
            categories.forEach(async (item, index) => {
              const productCategoryRelation = transactionalEntityManager.create(
                ProductCategoryRelationEntity,
                {
                  productId: savedProduct.id,
                  productCategoryId: item.id,
          ...
          if (dto.product_spea.length > 0) {
            ...
            const savedProductSpecs = await transactionalEntityManager.save(
              productSpecEntities,
            );
          ...
          for (const attr of dynamicAttributeList) {
            ...
            const existingRecord = await transactionalEntityManager.findOne(
              ProductDynamicAttributeEntity,
              {
                where: {
                  productId: savedProduct.id,
                  attributeId: attr.id,
                  attributeValueId: Array.isArray(attr.attributeValue)
            ...
            if (!existingRecord) {
              ...
              if (Array.isArray(attr.attributeValue)) {
                for (const value of attr.attributeValue) {
                  console.log(
                    `Inserting value: ${value} for attributeId: ${attr.id}`,
                  );
                  const productDynamicAttributeRelation =
                    transactionalEntityManager.create(
                      ProductDynamicAttributeEntity,
                ...
                console.log(
                  `Inserting single value: ${attr.attributeValue} for attributeId: ${attr.id}`,
                );
                const productDynamicAttributeRelation =
                  transactionalEntityManager.create(
                    ProductDynamicAttributeEntity,
              ...
              console.warn(
                `Skipping duplicate record for attributeId: ${attr.id}`,
              );
            }
          }
          return savedProduct;
