...
export class HoursService {
    constructor(
      ...
      async queryShopServingTime(data:any): Promise<any> {
        try {
            const storeHours = await this.businessHoursRepository.find({where:{storeId:data.storeId}});
                        
            let weeks = {0:'周日',1:'周一',2:'周二',3:'周三',4:'周四',5:'周五',6:'周六'}
       ...
       const store = await this.storeRepository.findOneBy({ id: data.storeId });
        ...
        return {
            ...
            shopStatusClickDetail: {
                ...
                operationDetailsForApp: this.getOperations(store, statusInfo.canOperate),
            },
            shopInfoVO: {
    ...
    // 核心状态判断
private calculateStatus(store: StoreEntity, businessHours: StoreHoursEntity[]) {
    // 1. 下线状态（最高优先级）
    if (store.online === 0) {
        return {
            status: 'offline',
...
private formatBusinessTime(businessHours: StoreHoursEntity[]) {
    ...
    // ✅ 详细格式：数字转中文后再拼接
    const detailList = Object.entries(timeGroups).map(([time, days]:[string, number[]]) => {
    ...
    const weeks = businessHours.map(h => h.dayOfWeek === 0 ? 7 : h.dayOfWeek);
